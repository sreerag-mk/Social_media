...
async function adminCheckForVerifying(admin, email) {
    try {
        const con = await connection.databaseConnection()
        let qr = 'SELECT * FROM admin as AD left join admin_permissions as AP on AD.id = AP.admin_id where admin_id = ? and AD.email = ? and permission_id = 4;'
